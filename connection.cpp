    ...
    , ui(new Ui::connection)
{
    ...
    // Add a "Create account" button to allow quick account creation (CIN + mdp)
    QPushButton *createBtn = new QPushButton(tr("Créer compte"), this);
    createBtn->setGeometry(200, 370, 120, 29);
    connect(createBtn, &QPushButton::clicked, this, &connection::onCreateAccount);
}
...
void connection::sendVerificationCodeByEmail(const QString &cin)
{
    ...
    // Brevo credentials now loaded securely from environment variable to avoid exposing secrets
    QString apiKey = qEnvironmentVariable("BREVO_API_KEY");
    QString senderEmail = "amineshimi90@gmail.com"; // You can also externalize this if needed

    if (apiKey.isEmpty()) {
        QMessageBox::warning(this, tr("Clé API manquante"), tr("Variable d'environnement BREVO_API_KEY non définie.\nDéfinissez-la avant d'envoyer un email."));
        return;
    }
...
void connection::onCreateAccount()
{
    bool ok;
    QString cin = QInputDialog::getText(this, tr("Créer compte"), tr("Entrez un CIN (8 caractères):"), QLineEdit::Normal, QString(), &ok);
    if (!ok || cin.isEmpty()) return;
    cin = cin.trimmed();
    if (cin.length() != 8) {
        QMessageBox::warning(this, tr("Erreur"), tr("Le CIN doit contenir exactement 8 caractères."));
    ...
    QString email = QInputDialog::getText(this, tr("Créer compte"), tr("Entrez l'email (optionnel):"), QLineEdit::Normal, QString(), &ok);
    if (!ok) return;
    email = email.trimmed();

    QString pwd = QInputDialog::getText(this, tr("Créer compte"), tr("Entrez le mot de passe:"), QLineEdit::Password, QString(), &ok);
    if (!ok) return;
    QString pwd2 = QInputDialog::getText(this, tr("Confirmer"), tr("Confirmez le mot de passe:"), QLineEdit::Password, QString(), &ok);
    if (!ok) return;
    if (pwd != pwd2) {
        QMessageBox::warning(this, tr("Erreur"), tr("Les mots de passe ne correspondent pas."));
        return;
    ...
    // Insert into UTILISATEUR (if exists) and into Users (for compatibility with login query)
    QSqlDatabase db = QSqlDatabase::database();

    // UTILISATEUR
    QSqlQuery q;
    q.prepare("INSERT INTO UTILISATEUR (cin, mdp, email, created_at) VALUES (:cin, :mdp, :email, SYSTIMESTAMP)");
    q.bindValue(":cin", cin);
    q.bindValue(":mdp", pwd); // NOTE: production => stocker hash
    q.bindValue(":email", email);
    if (!q.exec()) {
        // If the table doesn't exist or other error, show message but continue to try Users
        qDebug() << "UTILISATEUR insert error:" << q.lastError().text();
    }

    // Users table (compatibility)
    if (db.tables().contains("Users", Qt::CaseInsensitive)) {
        QSqlQuery q2;
        q2.prepare("INSERT INTO Users (cin, mdp, email) VALUES (:cin, :mdp, :email)");
        q2.bindValue(":cin", cin);
        q2.bindValue(":mdp", pwd);
        q2.bindValue(":email", email);
        if (!q2.exec()) {
            qDebug() << "Users insert error:" << q2.lastError().text();
            QMessageBox::warning(this, tr("Erreur"), tr("Impossible de créer le compte dans la table Users: %1").arg(q2.lastError().text()));
            return;
        }
    }

    QMessageBox::information(this, tr("Compte créé"), tr("Le compte a été créé. Vous pouvez maintenant vous connecter."));
}
