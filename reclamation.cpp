...
static const char* kTable = "FIRAS.RECLAMATION";
...
{
    if (code_ <= 0) {
        if (err) *err = "CODE doit être > 0.";
        return false;
    }
    if (!dateReclam_.isValid()) {
        if (err) *err = "DATE_RECLAMATION invalide.";
        return false;
    }
    if (nomResident_.trimmed().isEmpty()) {
        if (err) *err = "NOM_RESIDENT est obligatoire.";
        return false;
    }
    if (description_.trimmed().isEmpty()) {
        if (err) *err = "DESCRIPTION est obligatoire.";
        return false;
    }
...
bool Reclamation::insert(QString* err, QSqlDatabase db) const
{
    ...
    q.bindValue(":date_reclamation", dateReclam_.startOfDay());
    q.bindValue(":nom_resident", nomResident_);
    q.bindValue(":description", description_);
    q.bindValue(":etat", etat_);
    q.bindValue(":delai",
                delai_.trimmed().isEmpty()
                    ? QVariant()
                    : QVariant(delai_.trimmed()));
